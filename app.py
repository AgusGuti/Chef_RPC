...
@app.route("/deleteReceta", methods=["DELETE"])
def deleteReceta():
    with grpc.insecure_channel(os.getenv("SERVER-JAVA-RPC")) as channel:
        stub = receta_pb2_grpc.RecetasServiceStub(channel)     
        response = stub.getAllRecetas(receta_pb2.Receta)()
    
    print("Greeter client received: " + str(response)) 
    return MessageToJson(response)

# Endpoint para eliminar usuario
@app.route("/deletePerfil", methods=["DELETE"])
def deleteReceta():
    with grpc.insecure_channel(os.getenv("SERVER-JAVA-RPC")) as channel:
        stub = user_pb2_grpc.UsersServiceStub(channel)     
        response = stub.getAllUsers(user_pb2.User)
